...
async function setupMemberUI() {
  ...
  const token = localStorage.getItem('token');
  try {
    const res = await fetch('/api/member/requirements', { headers: { 'Authorization': `Bearer ${token}` } });
...
async function fetchMessages() {
  ...
  const token = localStorage.getItem('token');
  try {
    const res = await fetch(`/api/messages/history?partnerId=${currentChatPartnerId}`, { headers: { 'Authorization': `Bearer ${token}` } });
...
async function saveProfile() {
  ...
  const token = localStorage.getItem('token');
  try {
    const res = await fetch('/api/user/profile', { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ name, profilePicture }) });
    if (res.ok) { 
...
async function updateReq(memberId, reqId, payload) {
  const token = localStorage.getItem('token');
  try {
    await fetch(`/api/agent/requirements/${reqId}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    const res = await fetch('/api/agent/members', { headers: { 'Authorization': `Bearer ${token}` } });
