...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (useFallback) {
      const reqs = getFallbackData().requirements.filter(r => r.memberId === decoded.id);
      return res.status(200).json(reqs);
    }
    
    const reqs = await Requirement.find({ memberId: decoded.id });
    res.status(200).json(reqs);
  } catch (error) {
    if (error.name === 'JsonWebTokenError') {
      return res.status(401).json({ error: 'Invalid token' });
    }
    console.error('Get member requirements error:', error);
