  ...
  try {
    ...
    if (useFallback) {
      ...
      const msgs = data.messages.filter(m => 
        (m.senderId === decoded.id && m.receiverId === partnerId) || 
        (m.senderId === partnerId && m.receiverId === decoded.id)
      );
      return res.status(200).json(msgs.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)));
    }
